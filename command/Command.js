...
class Command {
     ...
     */
    constructor(userId, message, admin = []) {
        this.userId = userId;
    ...
    async getBotBindInfo(arg, nedb) {
        let apiObjects = arg.getOsuApiObject();
        return await new UserInfo(this.host).bindUser(nedb, this.userId, apiObjects[0]);
    }
    ...
    async getBotSetmodeInfo(arg, nedb) {
        let apiObjects = arg.getOsuApiObject();
        return await new UserInfo(this.host).setMode(nedb, this.userId, apiObjects[0].m);
    }
}
